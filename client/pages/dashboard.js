...
export default function Home() {
  ...
  function onSearch(event) {
    const newFilter = allElections.table
      ?.map((e, i) => (i == searchword ? i : undefined))
      .filter((value) => {
        return value;
      });
    if (searchword === "") {
        ...
        {!searchword ? (
          <div>
            <ResultCardDashboard
              ...
              color="bg-yellow-500"
              expandedOn={expandedPre}
              setExpandedOn={setExpandedPre}
          ...
          filtereddata?.map((election, ind) => (
            <li className="mx-4" key={election.id}>
              <ElectionCard
                ...
                imageURL={speech}
                description={
                  "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nullam id justo rutrum,"
                }
